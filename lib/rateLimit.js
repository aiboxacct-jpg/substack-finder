...
// Total free matches per day across everyone (bill safeguard). Members are
// unlimited and never count against this. ~70 ≈ $5/day at ~7¢ per match.
// Launch bump from 10; lower again once real usage settles.
const DAILY_CAP = 70;
...
const FREE_DAILY_LIMIT = 3; // fresh searches per day for a free/anon user

const freeHits = new Map(); // identity (user id, or IP if logged out) -> { day, count }

export function checkFreeDailyLimit(identity) {
  const today = todayKey();
  const rec = freeHits.get(identity);
  if (!rec || rec.day !== today) {
    freeHits.set(identity, { day: today, count: 1 });
